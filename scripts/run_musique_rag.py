"""Run a document-RAG baseline on a prepared MuSiQue pilot set."""
...
def _context_text(context: Dict[str, Any]) -> str:
    return " ".join(str(sentence) for sentence in context.get("sentences", []))
...
def _answer_question(example, *, model: str, top_k: int) -> Dict[str, Any]:
    ...
    result = chat_completion(
        [
            {
                "role": "system",
                "content": "You answer multi-hop questions from supplied evidence with concise gold-style answers. Return only the answer string.",
            },
...
def main() -> None:
    ...
    parser.add_argument("--output", default="evaluation/results/musique_rag_20.json")
    ...
    for index, example in enumerate(examples, start=1):
        if example.question_id in completed:
            print(f"[{index}/{len(examples)}] skip {example.question_id}", flush=True)
            continue
        print(f"[{index}/{len(examples)}] rag_basic {example.question_id}: {example.question}", flush=True)
        row = _answer_question(example, model=args.model, top_k=args.top_k)
        rows.append(row)
        _write_output(output, args=args, rows=rows)
        print(
            f"[{index}/{len(examples)}] answer={row['answer']!r} gold={row['gold_answer']!r} "
