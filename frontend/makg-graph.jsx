// makg-graph.jsx — D3 force-directed graph canvas for MaKG

const { useEffect, useRef, useState, useMemo } = React;

const { ENTITY_TYPES } = window.MAKG_DATA;
...
function GraphCanvas({
  ...
  const nodeGrpRef    = useRef(null);   // d3 selection of node <g> elements
  const linkGrpRef    = useRef(null);   // d3 selection of <line> elements
  ...
  // Stable callback refs to avoid stale closures inside D3 handlers
  ...
  // ── connected neighbors for detail panel ──────────────────────────────────
  ...
  useEffect(() => {
    ...
    // pulse ring (animated on chip hover)
    ...
    // labels: show for degree ≥ 3 or if tweaks say show all
    ...
    return () => sim.stop();
  }, [entities, triples]);  // eslint-disable-line
    ...
    <div style={{ position: 'relative', width: '100%', height: '100%', background: '#0f1117', overflow: 'hidden' }}>
      ...
      {tooltip && (
        <div style={{
          ...
          <div style={{
            background: '#1e2334', border: '1px solid rgba(124,131,232,0.3)',
            borderRadius: 4, padding: '10px 14px', minWidth: 200, maxWidth: 260,
            boxShadow: '0 8px 32px rgba(0,0,0,0.5)',
          }}>
            {tooltip.kind === 'node' && (() => {
              const { d, degree } = tooltip;
              const typeInfo = ENTITY_TYPES[d.type] || {};
              return (
                <>
      ...
      <div style={{
        ...
        {detailNode && (() => {
          const typeInfo = ENTITY_TYPES[detailNode.type] || {};
          ...
          return (
            <>
              ...
              <button onClick={() => { setDetailNode(null); onNodeClick(null); }}
                style={{ position: 'absolute', top: 12, right: 16, background: 'none', border: 'none', color: '#3d4555', cursor: 'pointer', fontSize: 16, padding: 4 }}>✕</button>
            </>
          );
        })()}
      </div>
