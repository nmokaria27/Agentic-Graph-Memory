...
| GB-9 | **Governed singlepass (SP-GOV)** — implemented (`extraction_mode="governed_singlepass"`, `--strategy spgov`, 259-test baseline). **EXP-SPGOV: REVERT as default** — entP +0.076 PASS but entR 0.726 vs 0.828 bar (organizer same-type over-merge on list-heavy docs: doc_109 29→9 entities, "Dr. Beat" absorbed 5 distinct albums), relF1 and wall bars also missed. Mode stays in-tree, opt-in | re-run **EXP-SPGOV-2** against the same bars after GB-11 lands — the over-merge is mechanically downstream of the type loss | blocked on GB-11 |
| GB-11 | **Entity types are stripped before stage 9 in ALL orchestrator modes** (spgov AND hybrid commit `type='?'` — verified on cached KGs) ⇒ **GB-8's type-compatibility gate has been inert in production since it shipped** (gate logic is correct + tested; it just never receives types). doc_109's "Spanish" absorbing USA/Cuban-American is exactly the cross-type merge it should have blocked | trace where `entity.get("type")` is dropped between wide/deliberative extraction and `_deduplicate_entities`/`_integrate_to_kg`; preserve it; fault-injection test that a committed KG entity retains its extraction type end-to-end. Small fix, benefits every mode | **NEXT (small)** |
