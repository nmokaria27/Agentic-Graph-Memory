...
| GB-2 | **Stale answers on knowledge updates** (EXP-ROBUST-VALIDATE final: 3/5 confirmed stale-serve — q0 27:12/25:50, q2 Chicago/suburbs, q3 $350k/$400k; **new sub-finding**: 2/5 — q1, q4 — are hedged non-answers despite the fact being present, coverage 0.75–0.88, a QA-synthesis issue distinct from `find_conflicts()`) | **EXP-FRESHNESS-QA**: (a) fix `find_conflicts()` batch-vs-incremental gap for the stale-serve pattern (q0/q2/q3); (b) investigate QA coverage/confidence synthesis separately for the hedge pattern (q1/q4) — may need its own sub-experiment if root causes diverge. Measure on knowledge-update dev, confirm held-out + no temporal regression | **NEXT** |
