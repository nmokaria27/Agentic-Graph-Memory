...
| GB-13 | **Fixed-schema enforcement leaks** (EXP-PAPER-COMPARE: non-SciERC types ORGANIZATION/PERSON/DATE in predictions under --fixed-schema; all score 0, hurt precision both legs) | trace where discovered/LLM types bypass the fixed-schema override; enforce at extraction + organizer; regression test | NEXT (small) |
| GB-14 | **Reviewer under-pruning on local models** (EXP-PAPER-COMPARE: governance keeps 68% of flat triples vs paper's 37% on GPT-5; strict P 0.051 vs 0.129) | tighten admission (evidence bar / confidence calibration) WITHOUT benchmark vocabulary; measure on SciERC dev slice then DocRED control | after GB-13 |
