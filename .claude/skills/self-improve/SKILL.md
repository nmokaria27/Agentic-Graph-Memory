...
| GB-2 | **Stale answers on knowledge updates** — root cause CORRECTED by EXP-FRESHNESS-QA: NOT `find_conflicts()` (it correctly sees intra-batch conflicts — verified by code trace) and NOT a missing resolver (wired by default). The real gap: `LLMConflictResolver`'s prompt never carried a recency signal (no document date reached triple provenance), so its own "if uncertain, coexist" fallback fired near-always (31–61 coexisting dupes vs 0–2 supersedes across 3 cached KGs; 3/5 EXP-ROBUST-VALIDATE answers stale: q0 27:12/25:50, q2 Chicago/suburbs, q3 $350k/$400k) | **EXP-FRESHNESS-QA: ACCEPT, merged.** `AgentContext.document_date` → `provenance.source_ref` → `_describe()` "date=" — unit + integration tested end-to-end (250-test baseline). Does NOT yet fix LongMemEval itself (nothing feeds it a date yet) — that's GB-2b | **mechanism CLOSED, symptom → GB-2b** |
| GB-2b | **LongMemEval never actually populates `document_date`** — `agent_graph_memory_adapter.py` concatenates all of a question's dated sessions into one blob, one `process_corpus([document])` call; GB-2's machinery has nothing to consume | per-session `process_corpus([{...}, ...])` calls, one per session, `metadata={"date": ...}`, **with `reuse_corpus_schema=True`** (avoid N schema-discovery calls/drift) **and `enable_cross_document=True`** (avoid the same real-world entity fragmenting into per-session ids, which would make `find_conflicts()` miss the conflict entirely) — needs its own cost + entity-id-stability success bar | after GB-2 (unblocked) |
| GB-9 | **Governed singlepass (SP-GOV)** — owner-approved architecture direction: per-segment single-call harvest → dedup → verify → govern, skipping RHF+deliberation. Target: singlepass recall + governed graph at ~2–5 min/doc | implement `extraction_mode="governed_singlepass"`; 3-way vs cached singlepass/hybrid; bars pre-drafted in EXPERIMENT_LOG discussion (entR ≥ 0.80, entP ≥ 0.83, relF1@0.6 ≥ 0.16, ≤ 5 min/doc) | after GB-8 + GB-2b |
