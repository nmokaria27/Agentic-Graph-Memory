...
| GB-9 | **Governed singlepass (SP-GOV)** — implemented (`extraction_mode="governed_singlepass"`, `--strategy spgov`, 259-test baseline). **EXP-SPGOV: REVERT as default** — entP +0.076 PASS but entR 0.726 vs 0.828 bar (organizer same-type over-merge on list-heavy docs: doc_109 29→9 entities, "Dr. Beat" absorbed 5 distinct albums), relF1 and wall bars also missed. Mode stays in-tree, opt-in | re-run **EXP-SPGOV-2** against the same bars now that GB-11 landed | **NEXT (unblocked)** |
