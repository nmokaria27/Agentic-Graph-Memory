...
| GB-2d | **Value-type collisions — CONFIRMED LIVE** (EXP-QA-PATH idx=3: KG holds loan_amount="$350,000", loan_amount_financed="$300,000", four_hundred_thousand_dollars="$400,000" (gold) via 3 different relations/entity shapes on one subject; find_conflicts() never fires — no shared key, so no supersede signal exists for QA to use) | domain-general value-shape collision detector (object-type/shape co-occurrence per subject, no relation-name assumptions) — pre-register design BEFORE code (GB-2c precedent) | **NEXT (evidence-grounded)** |
...
| GB-12 | **Distinct numeric/date literals merge in stage-9 dedup** (doc_100: 1911/1992 absorbed by 1939 — same type `DATE`, size-legal group, so GB-8's gate is correctly permissive; same exposure in hybrid) | **EXP-LITERAL-GUARD: ACCEPT** (0015386) — `literal_conflict` blocks distinct-digit merges in both stage-9 paths; diagnostic doc_100 entR 0.462→0.808, 21→42 committed, all years survive. Root path was the EMBEDDING tier of semantic dedup (years clear 0.85 embed sim, not 0.88 trigram). 270-test baseline | **CLOSED** |
| GB-13 | Fixed-schema type leak (5.6% of entities) | **KILLED by DIAG-SCHEMA-ADMIT** — removal ceiling ≈ 0 on triple F1; folds into GB-5 structured decoding (enum types) for free | parked |
| GB-14 | **Reviewer/extraction QUALITY gap vs paper** (DIAG-SCHEMA-ADMIT: strict F1 FLAT along the whole confidence frontier — calibration uninformative; paper's GPT-5 edge was precision AT high recall) | design first, pre-register before code: evidence-quote-grounded verification / LARGE-tier review / model headroom (gpt-oss-120b, SciERC dev slice). Free candidate: ≥0.90 admission floor (+14% mapped F1, needs DocRED control) | blocked on design |
| GB-3 | **Pair discovery is the binding constraint** (~23% of gold pairs found; relation naming is NOT the problem — EXP-JUDGE-PHASE4). Cross-lane evidence from EXP-FRESHNESS-E2E: Fireworks flash missed extracting q2's "suburbs" update entirely (0 entities for it) and modeled q0's update as a disjoint goal-entity instead of updating the existing one — a live example of the recall gap, on the Fireworks lane specifically (local Qwen3 got both right) | offline coverage analysis on Phase-4 caches, then 2-sample harvest union (#7) / targeted re-glean (#2) | **EXP-PAIR-COMPLETE: mechanism VALIDATED, config REVERTED** — pairR 0.242→0.305 (bar PASS) but model links ~every candidate pair (1897/1860 — omission instruction ignored) → entP −0.056, relF1 0.143, wall +84% all MISS. Stage 4c in-tree, default OFF. **GB-3b NEXT:** deterministic evidence-quote grounding filter (structural omission, zero LLM) + dumps must keep source/evidence fields | GB-3b NEXT (small) |
| GB-4 | **Extraction cost/latency** | **CLOSED** — Qwen3 swap (8 s/doc singlepass) + EXP-ASYNC-BATCH-1/2 `map_batches` fan-out (verification/coref/evidence/connectivity; −38% wall Fireworks, −20% local, EXP-SPGOV-3) + embedding failover to Fireworks on primary-ladder exhaustion (validated live). `LLM_BATCH_CONCURRENCY` default-off | **CLOSED** |
