...
We additionally evaluate the governed graph as memory for question answering. We compare four configurations on the same 50-document governed KG: \texttt{flat\_path\_basic} (a flat RAG-style path-and-context retrieval baseline); \texttt{graphrag\_basic} (a GraphRAG-style baseline that builds entity-relation communities and answers via community-summary retrieval, with no governance routing or critic); \texttt{domain\_basic} (governed domain router only); and \texttt{domain\_advanced} (governed router + active graph exploration + multi-agent debate + critic + provenance-aware memory). All four systems use the same underlying KG and the same 8 questions covering single-hop, multi-hop, comparison, negative, and cross-domain types. We score answers with KGAFE, a KG-grounded atomic-fact evaluation framework that decomposes each answer into atomic claims and verifies each claim against the governed graph.
...
\midrule
KGAFE score & 0.484 & 0.448 & 0.496 & \textbf{0.560} \\
KG faithfulness & 0.625 & 0.609 & 0.708 & \textbf{0.750} \\
KG precision & 0.625 & 0.609 & 0.646 & \textbf{0.741} \\
\textbf{Hallucination rate} & $37.5\%$ & $26.6\%$ & \textbf{$22.9\%$} & $25.9\%$ \\
Coverage & 0.938 & 0.750 & 0.812 & \textbf{1.000} \\
Total facts evaluated & 20 & 14 & 30 & 26 \\
\bottomrule
\end{tabular}
\caption{Four-way QA ablation on the 50-document governed KG ($n=8$ questions). The four systems form a clean \emph{flat $\to$ graph-aware $\to$ governed $\to$ governed+critic} ladder. Hallucination drops monotonically. Only governance simultaneously lifts faithfulness, precision, and KGAFE; \texttt{domain\_advanced} is the unique system at full coverage with the highest scores on every quality axis.}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Contrast} & $\Delta$ \textbf{KGAFE} & \textbf{$95\%$ CI} & \textbf{W / L / T} & \textbf{$p$} \\
\midrule
\texttt{graphrag\_basic} $-$ flat & $-0.037$ & $\boldsymbol{[-0.075,\,-0.009]}$ & $0\,/\,3\,/\,5$ & $0.250$ \\
\texttt{domain\_basic} $-$ flat & $+0.012$ & $[-0.067,\,+0.115]$ & $1\,/\,3\,/\,4$ & $0.625$ \\
\textbf{\texttt{domain\_advanced} $-$ flat} & \textbf{$+0.076$} & $[-0.005,\,+0.216]$ & $2\,/\,1\,/\,5$ & $1.000$ \\
\bottomrule
\end{tabular}
\caption{Paired sign-test on $\Delta$KGAFE for each baseline against \texttt{flat\_path\_basic} ($n=8$). The GraphRAG-style baseline's confidence interval excludes zero in the wrong direction: graph-aware retrieval alone helps the model abstain (coverage drops to $0.75$) but does not improve grounded faithfulness.}
\label{tab:qa-paired}
\end{table}
...
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \texttt{flat\_path\_basic} & \texttt{graphrag\_basic} & \texttt{domain\_basic} \\
\midrule
KGAFE score (judge-adjusted) & 0.765 & 0.550 & \textbf{0.850} \\
KG faithfulness & 0.600 & 0.600 & \textbf{0.867} \\
KG precision & 0.600 & 0.600 & \textbf{0.760} \\
\textbf{Hallucination rate} & $40.0\%$ & \textbf{$0.0\%$} & $24.0\%$ \\
Coverage & 0.900 & \textbf{$0.267$} & 0.667 \\
\bottomrule
\end{tabular}
\caption{Judge-panel rerun on the four hardest question types ($n=5$). The KGAFE judge panel re-scores each atomic-fact verdict end-to-end. Under strict judge grading, GraphRAG-style retrieval collapses to abstention: coverage drops to $0.27$ (it answers only $\sim 1.3$ of $5$ questions). The paired $\Delta$KGAFE for \texttt{graphrag\_basic} vs flat is $-0.215$ with $95\%$ CI $[-0.39,\,-0.05]$ (excludes zero in the wrong direction). Governance lifts faithfulness $+27$ pp and precision $+16$ pp, the largest deltas observed in any QA run.}
\label{tab:qa-judge}
\end{table}

In an additional supporting run on the same eight hard questions (multi-hop, comparison, negative, cross-domain) without the judge panel, \texttt{domain\_advanced} drives KGAFE to $0.665$, faithfulness to $1.000$, precision to $0.896$, and hallucination to $10.4\%$, with a paired $\Delta$KGAFE of $+0.180$ vs flat and a $95\%$ CI of $[+0.022,\,+0.381]$ that excludes zero. The single largest per-question lift is on the \texttt{negative} type ($0.15 \to 0.66$), where the advanced critic catches unverifiable claims that the flat baseline asserts.
...
\begin{enumerate}[nosep, leftmargin=*]
    ...
    \item \textbf{Governance helps downstream QA --- beyond what graph-aware retrieval alone delivers.} The four-way comparison shows hallucination dropping monotonically across flat $\to$ graph-aware $\to$ governed (Table~\ref{tab:qa-fourway}), but the GraphRAG-style baseline actually \emph{loses} on KGAFE in both runs (Tables~\ref{tab:qa-paired} and \ref{tab:qa-judge}, with $95\%$ CIs that exclude zero in the wrong direction). Only governance simultaneously lifts faithfulness, precision, and KGAFE; under strict judge grading the gap widens further (faithfulness $+27$ pp, precision $+16$ pp). Graph structure alone is not enough --- governance is what produces both high coverage and high faithfulness.
\end{enumerate}

\section{Related Work}
\label{sec:related}

\paragraph{Multi-agent KG construction.} Recent work such as KARMA~\cite{karma2025} studies multi-agent role specialization for knowledge graph construction. Our work differs in emphasis: the central object is not the extraction pipeline alone, but a governed graph with explicit ownership, admission control, and auditability.
