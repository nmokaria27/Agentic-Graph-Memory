...
LOG=evaluation/results/exp_async_batch2_fw.log
mkdir -p evaluation/results

{
  echo "[ASYNCB2] start $(date) model=$LLM_DEFAULT_MODEL commit=$(git rev-parse --short HEAD)"
  echo "[ASYNCB2] === arm A: LLM_BATCH_CONCURRENCY=1 (control) ==="
  LLM_BATCH_CONCURRENCY=1 python -u evaluation/DocRED/run_eval.py --strategy spgov \
    --offset 100 --max-docs 5 \
    --save-kg-dir evaluation/results/docred_kg_cache_fw_async2A \
    --output evaluation/results/exp_async_batch2_armA.json
  echo "[ASYNCB2] === arm B: LLM_BATCH_CONCURRENCY=4 ==="
  LLM_BATCH_CONCURRENCY=4 python -u evaluation/DocRED/run_eval.py --strategy spgov \
    --offset 100 --max-docs 5 \
    --save-kg-dir evaluation/results/docred_kg_cache_fw_async2B \
    --output evaluation/results/exp_async_batch2_armB.json
  echo "ASYNCB2_DONE $(date)"
} >> "$LOG" 2>&1
