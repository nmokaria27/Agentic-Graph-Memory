...
LOG=$OUT/matrix_v4.log

echo "[MATRIX4] start $(date)" >> "$LOG"
rm -f "$A"/doc_*_rhf.json "$A"/doc_*_hybrid.json \
      "$B"/doc_*_rhf.json "$B"/doc_*_hybrid.json
echo "[MATRIX4] stale rhf/hybrid caches cleared (singlepass kept)" >> "$LOG"

run() {  # run <slice-dir> <offset> <strategy>
    python -u evaluation/DocRED/run_eval.py --max-docs 5 --offset "$2" --strategy "$3" \
        --save-kg-dir "$1" --output "$OUT/docred_matrix4_$3_off$2.json" >> "$LOG" 2>&1
    echo "[MATRIX4] MILESTONE done: $3 offset=$2 $(date)" >> "$LOG"
}
...
for DIR in "$A" "$B"; do
    ...
    for S in rhf singlepass hybrid; do
        python evaluation/DocRED/score_docred.py --kg-dir "$DIR" --strategy "$S" \
            --output "$OUT/docred_scores_v4_${S}_${TAG}.json" >> "$LOG" 2>&1 \
        || python evaluation/DocRED/score_docred.py --kg-dir "$DIR" --strategy "$S" --no-embed \
            --output "$OUT/docred_scores_v4_${S}_${TAG}_noembed.json" >> "$LOG" 2>&1
    done
done

echo "[MATRIX4] MATRIX4_DONE $(date)" >> "$LOG"
