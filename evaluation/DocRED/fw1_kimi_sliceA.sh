#!/usr/bin/env bash
# EXP-1: model-headroom baseline — slice A (docs 0-4) on Fireworks kimi-k2p6.
...
export LLM_DEFAULT_MODEL=accounts/fireworks/models/kimi-k2p6
export VLLM_MAX_MODEL_LEN=131072

LOG=evaluation/results/fw1_kimi_sliceA.log
CACHE=evaluation/results/docred_kg_cache_fw_kimi
mkdir -p "$CACHE"

{
  ...
  for strat in singlepass hybrid; do
    ...
    python -u evaluation/DocRED/run_eval.py \
      ...
      --save-kg-dir "$CACHE" \
      --output "evaluation/results/fw1_kimi_${strat}.json"
  done
  echo "[FW1] scoring..."
  for strat in singlepass hybrid; do
    python evaluation/DocRED/score_docred.py \
      --kg-dir "$CACHE" --strategy "$strat" \
      --output "evaluation/results/fw1_kimi_scores_${strat}.json"
  done
  echo "FW1_DONE $(date)"
} >> "$LOG" 2>&1
