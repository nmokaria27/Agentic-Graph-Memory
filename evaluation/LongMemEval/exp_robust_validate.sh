...
LOG=evaluation/results/exp_robust_validate.log
CACHE=evaluation/results/lme_kg_cache_fw_validate
mkdir -p "$CACHE"

{
  echo "[EXPRV] start $(date) model=$LLM_DEFAULT_MODEL embed=$EMBEDDING_MODEL"
  python -u evaluation/LongMemEval/run_eval.py \
    --question-type knowledge-update --max-questions 5 \
    --save-kg-dir "$CACHE" \
    --output evaluation/results/exp_robust_validate.json
  echo "[EXPRV] scoring (substring only; judge later against same-lane model)..."
  python evaluation/LongMemEval/score_longmemeval.py \
    --cache-dir "$CACHE" \
    --output evaluation/results/exp_robust_validate_scores.json
  echo "EXPRV_DONE $(date)"
} >> "$LOG" 2>&1
