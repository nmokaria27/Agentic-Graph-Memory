...
# Fireworks lane (deepseek-v4-pro + qwen3-embedding-8b). Zero gpu02 usage.
set -uo pipefail
cd "$(dirname "$0")/../.."
...
export LLM_DEFAULT_MODEL=accounts/fireworks/models/deepseek-v4-pro
...
LOG=evaluation/results/exp2_lme_smoke.log
CACHE=evaluation/results/lme_kg_cache_fw_smoke
mkdir -p "$CACHE"

{
  echo "[EXP2] start $(date) model=$LLM_DEFAULT_MODEL embed=$EMBEDDING_MODEL"
  python -u evaluation/LongMemEval/run_eval.py \
    --question-type knowledge-update --max-questions 5 \
    --save-kg-dir "$CACHE" \
    --output evaluation/results/exp2_lme_smoke.json
  echo "[EXP2] scoring (substring only; judge later against same-lane model)..."
  python evaluation/LongMemEval/score_longmemeval.py \
    --cache-dir "$CACHE" \
    --output evaluation/results/exp2_lme_smoke_scores.json
  echo "EXP2_DONE $(date)"
} >> "$LOG" 2>&1
